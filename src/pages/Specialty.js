...
import { DisplaySpecialty } from '../components/DisplaySpecialty';
...
export default function Specialty() {
	...
	if (SpecialityCtx.Speciality != null && SpecialityCtx.Speciality !== '') {
	//	configurationOptions.searchQuery.query = SpecialityCtx.Speciality;
		configurationOptions.searchQuery.filters[0].values = [ SpecialityCtx.Speciality ];
	}

	// const [area,setarea]=useState();
	// useEffect(()=>{
	// 	setarea(AreaofInterestCtx.AreasofInterests);
	// },[AreaofInterestCtx.AreasofInterests, area])
	return (
		<SearchProvider config={configurationOptions}>
			<div className="App">
				<WithSearch
					mapContextToProps={({ searchTerm, reset,isLoading, filters, setSearchTerm, results, totalResults,clearFilters }) => ({
						searchTerm,
						isLoading,
						filters,
						setSearchTerm,
				...
				>
					{({ results, filters, setSearchTerm, searchTerm, totalResults ,reset,clearFilters}) => {
						...
						return (
							<div className="outer-container-mainpage">
								...
								<div className="resultsInfo">
									...
									{SpecialityCtx.Speciality && 
									<div className='searched-term'>
									<i>{SpecialityCtx.Speciality}</i>
										<button onClick={()=>{
											SpecialityCtx.setSpecialityHandler(null);
								...
								<div className="filtersAndMainContent">
									...
									<div className="main-content">
										<div className="kol-display-content">
                                            ...
                                            <DisplaySpecialty Results = {result1} />
                                            <DisplaySpecialty Results = {result2} />
										</div>
									</div>
								</div>
